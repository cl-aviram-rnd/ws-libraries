...
export class InputGroupExample extends React.PureComponent<IExampleProps, IInputGroupExampleState> {
  ...
  handleLargeChange = handleBooleanChange(large => this.setState({ large, ...(large && { small: false }) }));
  handleSmallChange = handleBooleanChange(small => this.setState({ small, ...(small && { large: false }) }));
  handleFilterChange = handleStringChange(filterValue => this.setState({ filterValue }));
  handleTagChange = handleStringChange(tagValue => this.setState({ tagValue }));
