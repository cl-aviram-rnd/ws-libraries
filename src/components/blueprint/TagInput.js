...
export default class TagInputExample extends React.PureComponent<IExampleProps, ITagInputExampleState> {
  ...
  handleAddOnBlurChange = handleBooleanChange(addOnBlur => this.setState({ addOnBlur }));
  handleAddOnPasteChange = handleBooleanChange(addOnPaste => this.setState({ addOnPaste }));
  handleDisabledChange = handleBooleanChange(disabled => this.setState({ disabled }));
  handleFillChange = handleBooleanChange(fill => this.setState({ fill }));
  handleIntentChange = handleBooleanChange(intent => this.setState({ intent }));
  handleLargeChange = handleBooleanChange(large => this.setState({ large }));
  handleLeftIconChange = handleBooleanChange(leftIcon => this.setState({ leftIcon }));
  handleMinimalChange = handleBooleanChange(minimal => this.setState({ minimal }));
  ...
  handleClear = () => this.handleChange(this.state.values.length > 0 ? [] : VALUES);
}